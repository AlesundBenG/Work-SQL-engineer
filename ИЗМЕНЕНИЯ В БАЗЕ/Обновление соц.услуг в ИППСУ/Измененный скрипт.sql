...
-------------------------------------------------------------------------------------------------------------------------------------
--Блок обработки услуг с сиделками.
IF (EXISTS (SELECT * FROM (SELECT DISTINCT OUID FROM #tmpu1) t INNER JOIN SPR_SOC_SERV ON SPR_SOC_SERV.OUID = t.OUID GROUP BY SPR_SOC_SERV.A_NAME HAVING COUNT(*) > 1)) BEGIN
    --Удаление временных таблиц.
    IF OBJECT_ID('tempdb..#REPEAT_SOC_SERV')        IS NOT NULL BEGIN DROP TABLE #REPEAT_SOC_SERV       END --Повторяющиеся по названию услуги.
    IF OBJECT_ID('tempdb..#SOC_SERV_IN_DOCUMENT')   IS NOT NULL BEGIN DROP TABLE #SOC_SERV_IN_DOCUMENT  END --Услуги, предоставляемые сиделками по договору.
    IF OBJECT_ID('tempdb..#SELECTED_SOC_SERV')      IS NOT NULL BEGIN DROP TABLE #SELECTED_SOC_SERV     END --Выбранные услуги.
    --Создание временных таблиц.
    CREATE TABLE #REPEAT_SOC_SERV (
        SOC_SERV_NAME VARCHAR(256) --Наименование услуги.
    )
    CREATE TABLE #SOC_SERV_IN_DOCUMENT (
        SOC_SERV_OUID   INT,
        SOC_SERV_NAME   VARCHAR(256)
    )
    CREATE TABLE #SELECTED_SOC_SERV (
        SOC_SERV_OUID INT, --Иденнти
    )
--////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    --Выбор повторяющихся наименований услуг.
    INSERT INTO #REPEAT_SOC_SERV(SOC_SERV_NAME)
    SELECT 
        socServ.A_NAME  AS SOC_SERV_NAME
    FROM (SELECT DISTINCT OUID FROM #tmpu1) t
    ----Справочник социальных услуг.
        INNER JOIN SPR_SOC_SERV socServ
            ON socServ.OUID = t.OUID 
    GROUP BY socServ.A_NAME 
    HAVING COUNT(*) > 1
--////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    --Договор, в котором перечислены услуги и подразделения, предоставляемые услуги.
    DECLARE @contract INT
    SET @contract = (
        SELECT TOP 1
            actDocuments_CONTRACT.OUID
        FROM INDIVID_PROGRAM individProgram --Индивидуальная программа.
        ----Связанный документ с индивидуальной программой.
            INNER JOIN WM_ACTDOCUMENTS actDocuments_IPPSU
                ON actDocuments_IPPSU.OUID = individProgram.A_DOC  
                    AND actDocuments_IPPSU.A_STATUS = 10
        ----Договор, в котором перечислены улсуги и подразделения, предоставляемые услуги.
            INNER JOIN WM_ACTDOCUMENTS actDocuments_CONTRACT
                ON actDocuments_IPPSU.PERSONOUID = actDocuments_CONTRACT.PERSONOUID
                    AND actDocuments_CONTRACT.A_STATUS = 10
                    AND CONVERT(DATE, actDocuments_IPPSU.ISSUEEXTENSIONSDATE) = CONVERT(DATE, actDocuments_CONTRACT.ISSUEEXTENSIONSDATE)
                    AND actDocuments_CONTRACT.DOCUMENTSTYPE IN (
        ...
        WHERE individProgram.A_OUID = @newProgId  
        ORDER BY actDocuments_CONTRACT.OUID DESC
    ) 
--////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    --Услуги по договору.
    INSERT INTO #SOC_SERV_IN_DOCUMENT (SOC_SERV_OUID, SOC_SERV_NAME)
    SELECT DISTINCT
        socServ.OUID    AS SOC_SERV_OUID,
        NULL            AS SOC_SERV_NAME
        --socServ.A_NAME  AS SOC_SERV_NAME
    FROM WM_SOC_SERV socServInDocument
    ----Тарифы на социальные услуги у организаций.
        INNER JOIN SPR_TARIF_SOC_SERV tarif
            ON tarif.A_ID = socServInDocument.A_TARIF
                AND tarif.A_STATUS = 10
    ----Справочник социальных услуг.
        INNER JOIN SPR_SOC_SERV socServ
            ON socServ.OUID = tarif.A_SOC_SERV
                AND socServ.A_STATUS = 10
    WHERE socServInDocument.A_CONTRACT = @contract
        AND socServInDocument.A_STATUS = 10
--////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    --Убираем те дубликаты, которых нет в договоре.
    DELETE FROM #tmpu1
    WHERE #tmpu1.OUID IN ( 
        SELECT DISTINCT 
            socServ.OUID 
        ...
        ----Справочник социальных услуг.
            INNER JOIN SPR_SOC_SERV socServ
                ON socServ.OUID = tmp.OUID 
        ----Повторяющиеся наименования.
            INNER JOIN #REPEAT_SOC_SERV repeatSocServ
                ON repeatSocServ.SOC_SERV_NAME = socServ.A_NAME
        ----Перечисленные в документе.
            LEFT JOIN #SOC_SERV_IN_DOCUMENT inDocument
                ON inDocument.SOC_SERV_OUID = socServ.OUID
        WHERE inDocument.SOC_SERV_OUID IS NULL --Нет в документе.
    )
END
-------------------------------------------------------------------------------------------------------------------------------------	
