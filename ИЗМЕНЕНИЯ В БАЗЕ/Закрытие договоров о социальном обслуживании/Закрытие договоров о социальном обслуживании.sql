...
WHERE actDocuments.A_STATUS = 10                                        --Статус в БД "Действует".
    ...
    AND (actDocuments.COMPLETIONSACTIONDATE IS NULL                     --Не закрыт.
        OR YEAR(actDocuments.COMPLETIONSACTIONDATE) >= 2021             --Или дата окончания стоит в новом периоде.
    ) 
...
WHERE socServ.A_STATUS = 10                             --Статус в БД "Действует".
    ...
    AND (period.A_LASTDATE IS NULL                      --Не закрыт период назначения.
        OR YEAR(period.A_LASTDATE) >= 2021              --Или дата окончания стоит в новом периоде.
    )
    AND (condition.A_LASTDATE IS NULL                   --Не закрыто условие.
        OR YEAR(condition.A_LASTDATE) >= 2021           --Или дата окончания стоит в новом периоде.
    )
...
SET actDocuments.TS = GETDATE(),                                        --Отмечаем время изменения.
    ...
    actDocuments.A_DOCBASEFINISHDATE = CONVERT(DATE, '31-12-2020'),     --Ставим дату окончания действия основания.
